...
def run_monotonic_reads_experiment(interactive=False):
    ...
    # 2. Background Writer Thread: Performs 30 updates on the Leader
    def writer_thread():
        conn = psycopg2.connect(**LEADER_DB)
        cur = conn.cursor()
        for version in range(2, 32):  # 30 updates
            ...
            # 100ms updates to let human observe the live checker
            time.sleep(0.1)
    ...
    # Read more times to cover the 3-second duration
    while not stop_event.is_set() or read_count < 100:
        ...
        # Very small wait between reads (1ms to catch the replication lag)
        time.sleep(0.001)
